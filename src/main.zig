...
const crcParams = struct {
    start: u32 = 0xFFFFFFFF,
    final: u32 = 0xFFFFFFFF,
    polynomial: u32 = 0x04C11DB7,
    check: u32 = 0xCBF43926,
    magic: u32 = 0xDEBB20E3,
    in_reflected: bool = true,
    out_reflected: bool = true,
}{};
...
pub fn table32normal() [256]u32 {
    var table: [256]u32 = undefined;
    const poly = crcParams.polynomial;
    for (0..256) |byte| {
...
test "table32reversed" {
    const t32r = table32reversed();
    try std.testing.expectEqualSlices(u32, &crc32t, &t32r);
}

pub fn crc32slowreflected(data: []const u8) u32 {
...
pub fn crc32slownormal(data: []const u8) u32 {
    var crc: u32 = crcParams.start;
...
test "crc32fastreflected" {
    ...
    const crc = crc32fastreflected(input);
    try std.testing.expectEqual(crc, crcParams.check);
    const crc_empty = crc32fastreflected("");
    const crc_empty_data: *const [4]u8 = @ptrCast(&crc_empty);
    const magic = crc32fastreflected(crc_empty_data) ^ crcParams.start;
    try std.testing.expectEqual(magic, crcParams.magic);
}

test "crc32slowreflected" {
    const input = "123456789";
    const crc = crc32slowreflected(input);
    try std.testing.expectEqual(crc, crcParams.check);
    const crc_empty = crc32slowreflected("");
    const crc_empty_data: *const [4]u8 = @ptrCast(&crc_empty);
    const magic = crc32slowreflected(crc_empty_data) ^ crcParams.start;
    try std.testing.expectEqual(magic, crcParams.magic);
}

test "crc32slownormal" {
    const input = "123456789";
    const crc = crc32slownormal(input);
    try std.testing.expectEqual(crc, crcParams.check);
    const crc_empty = crc32slownormal("");
    const crc_empty_data: *const [4]u8 = @ptrCast(&crc_empty);
